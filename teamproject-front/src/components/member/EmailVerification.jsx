...
const Signup = ({switchToLogin}) => {
    const [formData, setFormData] = useState({
        ...
        verificationCode: '' // 추가: 이메일 인증 코드 필드
    });

    const [errors, setErrors] = useState({});
    const [profilePreview, setProfilePreview] = useState('');
    const [modalIsOpen, setModalIsOpen] = useState(false);
    const [isEmailSent, setIsEmailSent] = useState(false); // 추가: 이메일 전송 상태
    const [isVerified, setIsVerified] = useState(false); // 추가: 이메일 인증 상태
    ...
    const handleSendVerificationCode = async () => {
        ...
        try {
            // 임시로 이메일 전송 성공 상태로 설정
            setIsEmailSent(true);
            setErrors({});
            console.log('인증 코드 전송 완료');
        } catch (error) {
            setErrors({email: '이메일 전송에 실패했습니다. 다시 시도해 주세요.'});
        }
    };

    const handleVerifyCode = async () => {
        if (!formData.verificationCode.trim()) {
            setErrors({verificationCode: '인증 코드를 입력해 주세요.'});
            return;
        }
        ...
        try {
            ...
            setErrors({verificationCode: '인증 코드 확인에 실패했습니다.'});
        }
    };
                                ...
                                <>
                                    <div className="signup-input-box">
                                        ...
                                        {errors.verificationCode &&
                                            <span className="signup-error-message">{errors.verificationCode}</span>}
                                    </div>
                                    {isVerified && <p>이메일 인증이 완료되었습니다.</p>}
                                </>
                            )}
                            <div className="signup-input-box">
                                                               <span className="signup-icon">
